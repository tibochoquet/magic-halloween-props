...
/**
 * Icon per profile. The URL is not here but in lib/companyInfo.ts, and an
 * entry without one is not rendered — see the filter below.
 */
const socials = [
  {
    name: "Instagram",
...
export default function Footer() {
  ...
  // Only profiles with a real URL in lib/companyInfo.ts.
  const activeSocials = socials.filter((s): s is typeof s & { href: string } => s.href !== null);
          ...
          <div className="col-span-2">
            ...
            {activeSocials.length > 0 && (
              <div className="flex items-center gap-3">
                {activeSocials.map((s) => (
                  <a key={s.name} href={s.href} target="_blank" rel="noreferrer" aria-label={s.name} className="w-9 h-9 border border-horror-border flex items-center justify-center text-horror-text-muted hover:text-horror-orange hover:border-horror-orange/40 hover:bg-horror-orange/5 transition-all duration-300">
                    {s.icon}
                  </a>
                ))}
              </div>
